...
const ESTADOS_OPCIONES = ['aceptado', 'preparando', 'listo_para_retirar', 'cancelado']
...
export default function Pedidos() {
  ...
  useEffect(() => {
    getPedidosEst()
      .then(data => {
        setPedidos(data)
        const init = {}
        data.forEach(p => { init[p.id_pedido] = 'aceptado' })
  ...
  const handleCambiarEstado = async (id_pedido) => {
    ...
    try {
      ...
      getPedidosEst().then(setPedidos).catch(() => {})
            ...
            <div key={p.id_pedido} className="bg-sidebar border border-gray-700 rounded-xl p-5">
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-2 ml-4">
                  <select
                    value={nuevosEstados[p.id_pedido] || 'aceptado'}
